...
import "../AdventureMovies/AdventureMovies.css";
...
const AdventureMovies = () =>{
    ...
    useEffect(()=>{
        ...
        const getAdventureMoviesData = async() =>{
         try{
            const response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=12`);
            ...
            const movieWithDetails = await Promise.all(
                data.results.map(async(movie)=>{
                    try{
                        ...
                        return{...movie, runtime, genres}
                    }
                    catch(error){
                        console.error('Error fetching adventure movies details', error);
                    }
                })
            )
            console.log(movieWithDetails);
            setAdventureMoviesData(movieWithDetails);
         }
         catch(error){
            console.error('Error fetching Adventure Movies Details', error);
         }
        }
        getAdventureMoviesData();
    },[]);
    ...
    const handleAdventureMoviesCard = (e) =>{
        let currentAdventureMovieCard = false;

        const adventureMoviesCard = document.querySelectorAll(".adventure-movie-poster");
        adventureMoviesCard.forEach((movieCard)=>{
            if(movieCard === e.currentTarget){
                currentAdventureMovieCard = true;
                return;
            }
            if(!currentAdventureMovieCard){
    ...
    const removeHandleAdventueMoviesCard = (e) =>{
        const adventureMoviesCard = document.querySelectorAll(".adventure-movie-poster");
        adventureMoviesCard.forEach((movieCard)=>{
    ...
    return(
        <>
            <section className="adventure-movies-section">
                ...
                <div className="adventure-movie-list">
                    <OwlCarousel className="owl-theme" {...options}>
                        {
                            adventureMoviesData && adventureMoviesData.map((movie)=>(
                                <Link key={movie.id} to={`/moviedetails/${movie.id}`}>
                                    <div key={movie.id} className="item adventure-movie-poster"
                                        onMouseEnter={handleAdventureMoviesCard}
                                        onMouseLeave={removeHandleAdventueMoviesCard}
                                        >
                                        <img className="adventure-movie-images" src={`https://image.tmdb.org/t/p/original/${movie.backdrop_path}`}
                                            alt={movie.title}
                                        />
