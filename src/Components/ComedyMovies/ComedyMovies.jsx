...
import "../ComedyMovies/ComedyMovies.css";
...
const ComedyMovies = () =>{
    ...
    useEffect(()=>{
        ...
        const getComedyMoviesData = async() =>{
            try{
                ...
                const movieWithDetails = await Promise.all(
                    data.results.map(async(movie)=>{
                        try{
                            ...
                            console.error('Error fetching comedy movie details', error);
                        }
                    })
                )
                console.log(movieWithDetails);
                setComedyMoviesData(movieWithDetails);
            }
            catch(error){
    ...
    const handleComedyMovieCard = (e) =>{
        let currentComedyMovieCard = false;

        const comedyMovieCard = document.querySelectorAll(".comedy-movie-poster");
        comedyMovieCard.forEach((movieCard)=>{
            if(movieCard === e.currentTarget){
                currentComedyMovieCard = true;
                return;
            }
            if(!currentComedyMovieCard){
    ...
    const removeHandleComedyMovieCard = () =>{
        const comedyMovieCard = document.querySelectorAll(".comedy-movie-poster");
        comedyMovieCard.forEach((movieCard)=>{
    ...
    return(
        <>
            <section className="comedy-movies-section">
                ...
                <div className="comedy-movie-list">
                    <OwlCarousel className="owl-theme" {...options}>
                        {
                            comedyMoviesData && comedyMoviesData.map((movie)=>(
                                <Link key={movie.id} to={`/moviedetails/${movie.id}`}>
                                    <div key={movie.id} className="item comedy-movie-poster"
                                        onMouseEnter={handleComedyMovieCard}
                                        onMouseLeave={removeHandleComedyMovieCard}
                                        >
                                        <img className="comedy-movie-images" src={`https://image.tmdb.org/t/p/original/${movie.backdrop_path}`} 
                                            alt={movie.title}
                                        />
