...
import "../CrimeMovies/CrimeMovies.css";
...
const CrimeMovies = () =>{
    const [crimeMoviesData, setCrimeMoviesData] = useState([]);
    ...
    useEffect(()=>{
        ...
        const getCrimeMoviesData = async() =>{
            try{
                const response = await axios(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=80`);
                const movieWithDetails = await Promise.all(
                    response.data.results.map(async(movie)=>{
                        try{
                            ...
                            console.error('Error fetching crime movie details', error);
                        }
                    })
                );
                console.log(movieWithDetails)
                setCrimeMoviesData(movieWithDetails);
            }
            catch(error){
                console.error('Error fetching crime movies data', error);
            }
        }
        getCrimeMoviesData();
    },[]);
    ...
    const handleCrimeMovieCard = (e) =>{
        let currentCrimeMovieCard = false;

        const crimeMovieCard = document.querySelectorAll(".crime-movie-poster");
        crimeMovieCard.forEach((movieCard)=>{
            if(movieCard === e.currentTarget){
                currentCrimeMovieCard = true;
                return;
            }
            if(!currentCrimeMovieCard){
    ...
    const removeHandleCrimeMovieCard = () =>{
        const crimeMovieCard = document.querySelectorAll(".crime-movie-poster");
        crimeMovieCard.forEach((movieCard)=>{
    ...
    return(
        <>
            <section className="crime-movies-section">
                ...
                <div className="crime-movie-list">
                    <OwlCarousel className="owl-theme" {...options}>
                        {
                            crimeMoviesData && crimeMoviesData.map((movie)=>(
                                <Link key={movie.id} to={`/moviedetails/${movie.id}`}>
                                    <div key={movie.id} className="item crime-movie-poster"
                                        onMouseEnter={handleCrimeMovieCard}
                                        onMouseLeave={removeHandleCrimeMovieCard}
                                        >
                                        <img className="crime-movie-images" src={`https://image.tmdb.org/t/p/original/${movie.backdrop_path}`} 
                                            alt={movie.title}
                                        />
